...
class ResourceLayoutWidget extends Widget
{
    public const NUMBER_OF_SLOTS = 19;

    public const VILLAGE_RESOURCE_BUILDINGS_LIST = 'villageResourceBuildingsList';
    public array $villageResourceBuildingsList;
        ...
        for ($i = 0; $i < self::NUMBER_OF_SLOTS;) {
            ...
            for ($j = 0; $j < $slotsInRow; $j++) {
                $elements .= $this->createSlot(
                    $i, 
                    $this->villageResourceBuildingsList[$i]['resource_type'], 
                    $this->villageResourceBuildingsList[$i]['level']
                );
                if ($this->villageResourceBuildingsList[$i]['resource_type'] == -1) { $slotsInRowTendency = -1; }
                $i++;
            }
            $elements .= '</div>';
    ...
    private function createSlot(int $slot, int $resourceType = -1, int $level = 0): string
    {
        $link = "#?slot=$slot";
        $className = '';
        $lvlText = "$level";
