...
class m231008_153137_create_queue_table extends Migration
{
    ...
    private const VILLAGE_FIELD_ID = 'id';

    private const QUEUE_TABLE_NAME = 'queue';
    private const QUEUE_FIELD_ID = 'id';
    private const QUEUE_FIELD_USER_ID = 'user_id';
    private const QUEUE_FIELD_PRODUCT_ID = 'product_id';
    private const QUEUE_FIELD_VILLAGE_ID = 'village_id';
    ...
    public function safeUp()
    {
        $this->createTable(self::QUEUE_TABLE_NAME, [
            ...
            self::QUEUE_FIELD_VILLAGE_ID => $this->integer()->notNull(),
            self::QUEUE_FIELD_PRODUCT_ID => $this->integer()->notNull(),
        ...
        $this->addForeignKey(
            self::QUEUE_TABLE_NAME . '_' . self::QUEUE_FIELD_VILLAGE_ID . '_fk',
            self::QUEUE_TABLE_NAME, self::QUEUE_FIELD_VILLAGE_ID,
            self::VILLAGE_TABLE_NAME, self::VILLAGE_FIELD_ID
        );

        $this->createIndex(
            self::QUEUE_TABLE_NAME . '_' . self::QUEUE_FIELD_USER_ID . '_' . self::QUEUE_FIELD_VILLAGE_ID . '_idx',
            self::QUEUE_TABLE_NAME, [ self::QUEUE_FIELD_USER_ID, self::QUEUE_FIELD_VILLAGE_ID ]
        );

        $this->db
            ->createCommand(
                'CREATE UNIQUE INDEX "' . self::QUEUE_TABLE_NAME . '_' . self::QUEUE_FIELD_USER_ID . '_' . self::QUEUE_FIELD_VILLAGE_ID . '_' . self::QUEUE_FIELD_QUEUE_TYPE . '_' . self::QUEUE_FIELD_COMMAND . '_' . self::QUEUE_FIELD_IS_PROCESSED . '_uidx" ' .
                'ON "' . self::QUEUE_TABLE_NAME . '" ' .
                '(' . self::QUEUE_FIELD_USER_ID . ',' . self::QUEUE_FIELD_VILLAGE_ID . ',' . self::QUEUE_FIELD_QUEUE_TYPE . ',' . self::QUEUE_FIELD_COMMAND . ',' . self::QUEUE_FIELD_IS_PROCESSED . ') ' .  
                'WHERE ' . self::QUEUE_FIELD_IS_PROCESSED . ' = false;'
            )->execute();
    }
    /**
     * {@inheritdoc}
